...
In our case we simply run from the zip file. \textbf{Java 17 is required to start the local instance of SonarQube}.\\
Our project details are : 
\begin{itemize}
    \item \textbf{Project-name and Project-key} : CKB-platform
    \item \textbf{Host-url} : http://localhost:9000
    \item \textbf{Token} : the token is generated once the server is running
\end{itemize}
...
Once we have installed everything and run the SonarQube server, we can modify our file \textbf{sonar-project.properties} with the right token; to analyse our code we have to open a terminal in our root directory and run the following command : 
\texttt{sonar-scanner.bat}.\\
Now we can see our analysis at our host-url.
